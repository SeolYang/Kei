...
namespace sy::render
{
	Renderer::Renderer(const window::Window& window, vk::VulkanContext& vulkanContext, vk::ResourceStateTracker& resStateTracker, ResourceCache& resourceCache) :
		window(window),
		...
		resourceCache(resourceCache)
	{
		...
		triFrag = std::make_unique<vk::ShaderModule>("Triangle fragment shader", vulkanRHI, "Assets/Shaders/bin/textured_tri_bindless.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT, "main");
		...
		renderPass = std::make_unique<SimpleRenderPass>("Simple Render Pass", resourceCache, vulkanContext, *basicPipeline);
	}
