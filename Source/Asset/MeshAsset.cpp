...
namespace sy::asset
{
	...
	std::unique_ptr<render::Mesh> LoadMeshFromAsset(std::string_view assetPath,
		...
		const vk::FrameTracker& frameTracker)
	{
		...
		const std::vector<char> verticesBytes = UnpackVertex(metadata, uncompressedData);
		const std::vector<render::IndexType> indices = UnpackIndex(metadata, uncompressedData);

		return render::Mesh::Create<render::VertexPTN, render::IndexType>(
			metadata.SrcPath,
			vulkanContext, cmdPoolManager, frameTracker,
			std::span{ reinterpret_cast<const render::VertexPTN*>(verticesBytes.data()), verticesBytes.size() / sizeof(render::VertexPTN) },
			indices);
	}
}
