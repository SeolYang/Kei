...
namespace sy
{
	namespace vk
	{
		Pipeline::Pipeline(std::string_view name, const VulkanRHI& vulkanRHI, const GraphicsPipelineBuilder& builder)
		...
		Pipeline::Pipeline(std::string_view name, const VulkanRHI& vulkanRHI, const ComputePipelineBuilder& builder)
		...
		Pipeline::Pipeline(std::string_view name, const VulkanRHI& vulkanRHI, EPipelineType pipelineType,
			...
			: VulkanWrapper<VkPipeline>(
				name, vulkanRHI, VK_OBJECT_TYPE_PIPELINE,
				VK_DESTROY_LAMBDA_SIGNATURE(VkPipeline) { vkDestroyPipeline(vulkanRHI.GetDevice(), handle, nullptr); })
