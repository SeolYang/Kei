...
namespace sy
{
	namespace vk
	{
			...
			Pipeline(name, vulkanRHI, EPipelineType::Graphics, builder.GetLayout())
		{
			...
			VK_ASSERT(vkCreateGraphicsPipelines(vulkanRHI.GetDevice(), VK_NULL_HANDLE, 1, &createInfo, nullptr, &handle), "Failed to create graphics pipeline {}.", name);
			UpdateHandle(handle);
		}
			...
			Pipeline(name, vulkanRHI, EPipelineType::Compute, builder.GetLayout())
		{
			const auto createInfo = builder.Build();
			Native_t handle = VK_NULL_HANDLE;
			VK_ASSERT(vkCreateComputePipelines(vulkanRHI.GetDevice(), VK_NULL_HANDLE, 1, &createInfo, nullptr, &handle), "Failed to create compute pipeline {}.", name);
			UpdateHandle(handle);
		}

		Pipeline::Pipeline(std::string_view name, const VulkanRHI& vulkanRHI, EPipelineType pipelineType, VkPipelineLayout layout) :
