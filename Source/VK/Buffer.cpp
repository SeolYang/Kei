...
namespace sy
{
	namespace vk
	{
		size_t CalculateAlignedBufferSize(const VulkanContext& vulkanContext, const size_t originSize,
			...
			initialState(builder.targetInitialState)
		{
			...
			VK_ASSERT(vmaCreateBuffer(vulkanRHI.GetAllocator(), &createInfo, &allocationCreateInfo, &handle, &allocation
				          , nullptr), "Failed to create buffer {}.", builder.name);
			UpdateHandle(handle);
